"""
Author: windzu
Date: 2022-03-26 10:36:55
LastEditTime: 2022-03-26 10:38:05
LastEditors: windzu
Description: 
FilePath: /monocular_camera_calibration/test/surround_view_node.py
...
class SurroundViewNode:
    ...
    def show_result(self):
        ...
        while True:
            ...
            # new_frame = tonemapMantiuk.process(frame)
            # new_frame = 3 * new_frame
            # new_frame = new_frame * 255

            cv2.imshow("result", frame)
            # cv2.imshoe("new_frame", new_frame)
            key = cv2.waitKey(1)
            if key == ord("q"):
                cv2.destroyAllWindows()
                break
    ...
    def _on_scale_x_trackbar(self, scale_x_progress):
        self.camera_info.scale_xy[0] = scale_x_progress / 100

    def _on_scale_y_trackbar(self, scale_y_progress):
        self.camera_info.scale_xy[1] = scale_y_progress / 100

    def _on_shift_x_trackbar(self, shift_x_progress):
        self.camera_info.shift_xy[0] = shift_x_progress - 300

    def _on_shift_y_trackbar(self, shift_y_progress):
        self.camera_info.shift_xy[1] = shift_y_progress - 300

    def _calculate_homography_matrix(self, src_points):
        if self.camera_id == "/camera/front_wild":
            dst_points = self.cc_info.dst_points_dict["front"]
        elif self.camera_id == "/camera/left_wild":
            dst_points = self.cc_info.dst_points_dict["left"]
        elif self.camera_id == "/camera/right_wild":
            dst_points = self.cc_info.dst_points_dict["right"]
        elif self.camera_id == "/camera/back_wild":
            dst_points = self.cc_info.dst_points_dict["back"]
        else:
            raise Exception("camera_id error")
    ...
    def _generate_camera_mask(self, mask_points, mask_size):
        ...
        return mask
        # return cv2.findHomography(src_points, dst_points, cv2.RANSAC, 5.0)
        # self.current_camera_info.homography_matrix = cv2.getPerspectiveTransform(src_points, dst_points)
