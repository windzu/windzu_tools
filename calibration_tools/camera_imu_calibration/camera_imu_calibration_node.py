...
class CameraIMUCalibrationNode:
    ...
    def __init__(self, get_frame, get_rtk, camera_info, tl_info):
        self.get_frame = get_frame  # uese to get img
        self.get_rtk = get_rtk  # use to get car pos
        self.camera_info = camera_info  # store camera params about intrinsics , distortion , imu_to_camera_extrinsics
        self.tl_info = tl_info  # store tl info about tl position
        ...
        self.__translation_slider_step_size = 100  # 缩放100倍
        self.__rotation_offset_slider_max = 900
        self.__rotation_offset_slider_step_size = 10  # 缩放100倍
    ...
    def start(self):
        ...
        while True:
            ...
            ret, center, roi_rect_pts = self.__calculate_roi(tl_info=tl_info, car_position=car_position, camera_info=camera_info)
            if ret is True:
    ...
    def __on_translation_x_trackbar(self, progress):
        ...
        self.camera_info.imu_to_camera_translation_xyz[0] = translation_x
    ...
    def __on_translation_y_trackbar(self, progress):
        ...
        self.camera_info.imu_to_camera_translation_xyz[1] = translation_y
    ...
    def __on_translation_z_trackbar(self, progress):
        ...
        self.camera_info.imu_to_camera_translation_xyz[2] = translation_z

    def __on_rotation_offset_x_trackbar(self, progress):
        rotation_offset_x = (
            (progress - (self.__rotation_offset_slider_max / 2)) * (math.pi / 180) / self.__rotation_offset_slider_step_size
        )
        self.camera_info.imu_to_camera_rotation_offset_xyz[0] = rotation_offset_x

    def __on_rotation_offset_y_trackbar(self, progress):
        rotation_offset_y = (
            (progress - (self.__rotation_offset_slider_max / 2)) * (math.pi / 180) / self.__rotation_offset_slider_step_size
        )
        self.camera_info.imu_to_camera_rotation_offset_xyz[1] = rotation_offset_y

    def __on_rotation_offset_z_trackbar(self, progress):
        rotation_offset_z = (
            (progress - (self.__rotation_offset_slider_max / 2)) * (math.pi / 180) / self.__rotation_offset_slider_step_size
        )
        self.camera_info.imu_to_camera_rotation_offset_xyz[2] = rotation_offset_z

    @staticmethod
    def __calculate_roi(tl_info, car_position, camera_info):
        """_summary_
        ...
        # 世界坐标到车辆imu坐标的变换
        # 先平移到车辆的中心，再围绕z轴旋转
        car_x = car_position.x
        car_y = car_position.y
        car_z = car_position.z
        car_yaw = car_position.yaw
        world_to_imu_translation_matrix = np.eye(4)
        world_to_imu_translation_matrix = np.array([[1, 0, 0, -car_x], [0, 1, 0, -car_y], [0, 0, 1, -car_z], [0, 0, 0, 1]])
        x_axis = [1, 0, 0]
        y_axis = [0, 1, 0]
        z_axis = [0, 0, 1]
        world_to_imu_rotation_matrix = rotate_mat(z_axis, -car_yaw, 4)
        world_to_imu_tranformation_matrix = np.dot(world_to_imu_rotation_matrix, world_to_imu_translation_matrix)
        ...
        camera_to_pixel_translation_matrix = np.concatenate((camera_to_pixel_translation_matrix, np.zeros((4, 1))), axis=1)
        camera_to_pixel_translation_matrix[3, 3] = 1

        world_to_pixel_tranformation_matrix = np.eye(4)
        world_to_camera_tranformation_matrix = np.dot(imu_to_camera_tranformation_matrix, world_to_imu_tranformation_matrix)
        world_to_pixel_tranformation_matrix = np.dot(camera_to_pixel_translation_matrix, world_to_camera_tranformation_matrix)
